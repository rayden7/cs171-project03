...
window.onload = function() {
    ...
    d3.csv("csv/rider-data-with-casualties2.csv", function(error, data) {
        data.forEach(function(d) {
            riderDataset.push({
                ...
                RiderCountry:d.RiderCountry,
                RiderCountryFlag:d.RiderCountryFlag,
                DeathDate:d.DeathDate,
                DeathYear: +d.DeathYear,
                DeathPlace: d.DeathPlace,
                DeathRace:d.DeathRace,
                DeathEvent:d.DeathEvent,
                DeathMachine:d.DeathMachine,
                YearsOfExperience:0
            });
        });
        ...
        //riderDataset = riderDataset.filter(function(d){ return d.RiderID == 10002});
        ...
        d3.csv("csv/races_data.csv", function(error, data) {
            ...
            // iterate over all the riders in "dataset", and calculate how many years of riding experience we have
            // for each rider - then set the "YearsOfExperience" property on that rider in "riderdataset"
            var raceRecordsByRider = d3.nest()
                .key(function(d) { return d.RiderID; })    // group all the records for the individual RiderID
                .key(function(d) { return d.Year; })    // group all the records by Year
                .entries( dataset );

            for(var i=0; i < raceRecordsByRider.length; i++) {
                var yearsExp = raceRecordsByRider[i].values.length;
                var sameRider = riderDataset.filter(function(d) { return d.RiderID == raceRecordsByRider[i].values[0].values[0].RiderID; });
                if (sameRider && sameRider.length >= 1) {
                    sameRider[0].YearsOfExperience = yearsExp;
                }
                //console.log("RiderID: ["+raceRecordsByRider[i].values[0].values[0].RiderID+"]  yearsExp: ["+yearsExp+"]   raceRecordsByRider[i].values[0].values[0].YearsExperience: ["+raceRecordsByRider[i].values[0].values[0].YearsOfExperience+"]   sameRider[0].YearsOfExperience: ["+sameRider[0].YearsOfExperience+"]");
            }

            //var johnMcGuiness = riderDataset.filter(function(d) { return d.RiderID == 4307; });

            var ridersThatHaveDied = d3.nest()
                .key(function(d) { return d.YearsOfExperience; })    // group all the records by Year
                .entries( riderDataset.filter(function(d) { return (d.DeathYear > 0 && d.DeathRace.indexOf("Isle of Man TT") > 0); }));
